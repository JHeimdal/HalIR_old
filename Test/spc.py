...
class SPC:
	def __init__(self,filename):
		...
		with open(filename,'rb') as file1:
			self.spchdr=struct.unpack('<BBBcLddLBBBBL9s9sH32s130s30sLLBBHf48sfLfB187s',file1.read(512))
			...
			if (self.spchdr[ftflgs]==0):
				...
				for i in range(len(self.ydata)):
					self.xdata.append(self.spchdr[ffirst]+i*(self.spchdr[flast]-self.spchdr[ffirst])/(self.spchdr[fnpts]-1))
